...
import { getPkmnData, responseData } from './api';
...
function tagFormat(id: string, tag = true) {
...
function hexConversion(hectogram: number) {
...
function dexConversion(decimeter: number) {
  const foot = 0.328084;
  ...
  let inches: string | number = Math.round(decimal / inch);
  if (inches < 10) {
...
function createCardHTML(data: responseData) {
  // create card div
...
function createModalHTML(data: responseData, speciesData: any) {
  ...
  try {
    // not all pokemon have two types, this will catch any errors
...
function createCardDOM(card: any, data: any, speciesData: any) {
...
async function createCard(id: number) {
